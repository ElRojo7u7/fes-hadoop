...
if [ "${NODES}" -le 0 ]; then echo -e "${Red}[!] Error${NC}: variable NODES has an unvalid value; please provide a valid value"; exit 1; fi
...
HostName ${HOSTNAME}"
    ...
    while [ "${NODES}" -ne "$(wc -l < /opt/hadoop/etc/hadoop/workers)" ]; do sleep 2; done
